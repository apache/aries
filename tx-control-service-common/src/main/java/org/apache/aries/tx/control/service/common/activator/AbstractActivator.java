...
public abstract class AbstractActivator implements BundleActivator, ServiceTrackerCustomizer<Coordinator, Coordinator> {
	...
	private void checkAndRegister(Coordinator c) {
		...
		if(register) {
			...
			ServiceRegistration<TransactionControl> reg = context.registerService(
					TransactionControl.class, getTransactionControl(c), getProperties());
			synchronized (lock) {
				this.reg = reg;
			}
		}
	...
	protected abstract TransactionControl getTransactionControl(Coordinator c);
